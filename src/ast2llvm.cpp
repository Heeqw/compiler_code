...
void ast2llvmBlock(aA_codeBlockStmt b, Temp_label* con_label, Temp_label* bre_label) {
    ...
    case A_codeBlockStmtType::A_varDeclStmtKind: {
        ...
        if (varDeclStmt->kind == A_varDeclStmtType::A_varDeclKind) {
            ...
            case A_varDeclType::A_varDeclScalarKind: {
                ...
                // // 将未定义类型设为 nullptr
                localVarMap[id] = nullptr;
             if(declScalar->type != nullptr){
                auto type = declScalar->type;
                switch (type->type) {
    ...
    case A_codeBlockStmtType::A_assignStmtKind: {
        ...
        //类型推断
        if(leftPtr->kind==OperandKind::TEMP){
            if(leftPtr->u.TEMP == nullptr){
            // leftPtr->u.TEMP->type = right->u.TEMP->type;
            auto leftId = *(assignStmt->leftVal)->u.id;
            if(localVarMap[leftId]==nullptr){
                //如果右值是TEMP
                if(right->kind==OperandKind::TEMP){
                    auto rightTemp=right->u.TEMP;
                    Temp_temp* temp = nullptr;
                    switch (rightTemp->type)
                    {
                        ...
                        case TempType::INT_PTR: {
                            temp = Temp_newtemp_int_ptr(rightTemp->len);
                        } break;

                        case TempType::STRUCT_TEMP: {
                            temp = Temp_newtemp_struct_ptr(0, rightTemp->structname);
                        } break;

                        case TempType::STRUCT_PTR: {
                            temp = Temp_newtemp_struct_ptr(rightTemp->len, rightTemp->structname);
                        } break;
                    ...
                    auto leftTemp=temp;
                    leftTemp->structname=leftId;
                    leftTemp->varname=leftId;
                    localVarMap[leftId]=leftTemp;
                    emit_irs.push_back(L_Alloca(AS_Operand_Temp(leftTemp)));
                    leftPtr = ast2llvmLeftVal(assignStmt->leftVal);
                }
                //如果右值是全局变量NAME
                else if(right->kind==OperandKind::NAME){
                    auto rightName=right->u.NAME;
                    Temp_temp* temp = nullptr;
                    switch (rightName->type)
                    {
                        case TempType::INT_TEMP: {
                            temp = Temp_newtemp_int_ptr(0);
                        } break;
                        ...
                        case TempType::INT_PTR: {
                            temp = Temp_newtemp_int_ptr(rightName->len);
                        } break;

                        case TempType::STRUCT_TEMP: {
                            temp = Temp_newtemp_struct_ptr(0, rightName->structname);
                        } break;

                        case TempType::STRUCT_PTR: {
                            temp = Temp_newtemp_struct_ptr(rightName->len, rightName->structname);
                        } break;
                ...
                //如果右值是立即数ICONST
                else if(right->kind==OperandKind::ICONST){
                    auto iconst=right->u.ICONST;
                    Temp_temp* temp = nullptr;
                    temp = Temp_newtemp_int_ptr(0);
                    auto leftTemp=temp;
                    leftTemp->structname=leftId;
